...
impl<'a> ItemIterator<'a> {
    ...
    pub fn next(
        ...
        if let Some(dragged_item_rect) = self.dragged_item_rect {
            // When comparing the dragged item rect with the current item rect, we need to take into account the layer transform
            let transform = ui.ctx().layer_transform_to_global(ui.layer_id());
            let global_space_rect = if let Some(transform) = transform {
                transform.mul_rect(rect)
            } else {
                rect
            };

            if self.layout.main_wrap {
                if global_space_rect.contains(dragged_item_rect.center()) {
                    if self.is_after_hovered_item {
                        self.mark_next_as_closest_item = Some((0.0, global_space_rect.min));
                    } else {
                        self.closest_item = Some((0.0, Some((idx, id, global_space_rect.min))));
                    }
                }
            } else {
