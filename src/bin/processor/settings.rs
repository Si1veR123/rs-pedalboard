...
pub struct ProcessorArguments {
    ...
    #[arg(
        long,
        help = "Number of periods of the minimum frequency to process for pitch (default: 5)"
    )]
    ...
    #[arg(
        long,
        help = "Preferred sample rate for the audio host. Uses highest if not available. (default: 48000)"
    )]
    pub preferred_sample_rate: Option<u32>,
    #[arg(
        long,
        help = "Number of 2x upsample passes to apply before processing (default: 0)"
    )]
    pub upsample_passes: Option<u32>,
    #[arg(
        long,
        default_value_t = false,
        help = "Ignore saved settings - use command line arguments/default"
    )]
    pub ignore_save: bool,
    #[arg(
        long,
        help = "Directory to save recordings to (default: ~/rs_pedalboard/Recordings)"
    )]
...
impl ProcessorSettings {
    pub fn new(args: ProcessorArguments, mut saved: Option<ProcessorSettingsSave>) -> Self {
        ...
        let host = match args.host {
            ...
            None => saved
                .as_ref()
                .map_or_else(|| SupportedHost::default(), |s| s.host.clone()),
        };
        ...
        let buffer_latency = args.buffer_latency.unwrap_or_else(|| {
            saved
                .as_ref()
                .map_or_else(|| constants::DEFAULT_RING_BUFFER_LATENCY_MS, |s| s.latency)
        });

        let periods_per_buffer = args
            .periods_per_buffer
            .unwrap_or_else(|| saved.as_ref().map_or_else(|| 3, |s| s.periods_per_buffer));

        let tuner_min_freq = args.tuner_min_freq.unwrap_or(40);
        let tuner_max_freq = args.tuner_max_freq.unwrap_or(1300);
        let tuner_periods = args
            .tuner_periods
            .unwrap_or_else(|| saved.as_ref().map_or_else(|| 5, |s| s.tuner_periods));
        let input_device = args
            .input_device
            .or_else(|| saved.as_ref().and_then(|s| s.input_device.clone()));
        let output_device = args
            .output_device
            .or_else(|| saved.as_ref().and_then(|s| s.output_device.clone()));

        let preferred_sample_rate = args
            .preferred_sample_rate
            .or_else(|| saved.as_ref().and_then(|s| s.preferred_sample_rate));

        let upsample_passes = args
            .upsample_passes
            .unwrap_or_else(|| saved.as_ref().map_or_else(|| 0, |s| s.upsample_passes));
        ...
        ProcessorSettings {
            ...
            recording_dir: Self::recording_dir(
                ...
                saved
                    .as_ref()
                    .and_then(|s| s.recording_dir.as_ref().map(|p| p.as_path())),
            ),
        }
    }
    ...
    pub fn default_recording_dir() -> Option<PathBuf> {
        let dir = homedir::my_home().ok().and_then(|p| {
            p.and_then(|p| Some(p.join(rs_pedalboard::SAVE_DIR).join("Recordings")))
        })?;
