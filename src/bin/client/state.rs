...
impl State {
    ...
    pub fn remove_pedalboard_from_stage(&self, index: usize, local: bool) {
        ...
        if pedalboard_set.active_pedalboard > index
            || pedalboard_set.active_pedalboard == pedalboard_set.pedalboards.len()
        {
    ...
    pub fn add_pedalboard(&self, pedalboard: Pedalboard, local: bool) {
        if !local {
            let mut socket = self.socket.borrow_mut();
            socket.send(Command::AddPedalboard(
    ...
    pub fn add_pedal_to_active(&self, pedal: &Pedal, local: bool) {
        ...
        let active_pedalboard_id =
            active_pedalboardstage.pedalboards[active_pedalboardstage.active_pedalboard].get_id();
    ...
    pub fn move_pedal(&self, pedalboard_id: u32, pedal_id: u32, mut to_index: usize, local: bool) {
        ...
        if src_index.is_none() {
            tracing::error!(
                "move_pedal: Could not find pedalboard with ID {} in pedalboard library",
                pedalboard_id
            );
            return;
        }
    ...
    pub fn play(&self, pedalboard_index: usize, local: bool) {
        ...
        let new_pedalboard_id =
            active_pedalboardset.pedalboards[active_pedalboardset.active_pedalboard].get_id();
        self.midi_state
            .borrow()
            .active_pedalboard_id
            .store(new_pedalboard_id, std::sync::atomic::Ordering::Relaxed);
    ...
    pub fn set_volume_normalization_processor(
        &self,
        mode: crate::settings::VolumeNormalizationMode,
        auto_decay: f32,
    ) {
        let mut socket = self.socket.borrow_mut();
        ...
        if matches!(mode, VolumeNormalizationMode::Automatic) {
            socket.send(Command::VolumeNormalization(
                VolumeNormalizationMode::Automatic,
                Some(rounded_auto_decay),
            ));
        } else {
    ...
    pub fn load_state(egui_ctx: eframe::egui::Context) -> Self {
        ...
        let ir_root_nodes: Vec<_> = client_settings
            .ir_folders
            .iter()
            .map(|p| egui_directory_combobox::DirectoryNode::from_path(p))
            .collect();
        ...
        egui_ctx.memory_mut(|writer| {
            writer
                .data
                .insert_temp(egui::Id::new("nam_folders_state"), 1u32);
            writer
                .data
                .insert_temp(egui::Id::new("nam_folders"), nam_root_nodes);
            writer
                .data
                .insert_temp(egui::Id::new("ir_folders_state"), 1u32);
            writer
                .data
                .insert_temp(egui::Id::new("ir_folders"), ir_root_nodes);
            writer
                .data
                .insert_temp(egui::Id::new("vst2_folders_state"), 1u32);
            writer
                .data
    ...
    pub fn connect_to_processor(&self) -> Result<(), std::io::Error> {
        ...
        if !socket.is_connected() {
            ...
            if socket.is_connected() {
                ...
                self.set_volume_normalization_processor(
                    client_settings.volume_normalization,
                ...
                self.set_metronome(
                    ...
                    self.metronome_bpm.get(),
                    self.metronome_volume.get(),
                );
                self.recording_time.set(None);
    ...
    pub fn set_screen(&self, screen: Screen) {
        ...
        if self.selected_screen.get() != screen {
            self.prev_selected_screen
                .set(Some(self.selected_screen.get()));
        }
        ...
        for command in self.midi_command_receiver.try_iter() {
            match command {
                Command::LoadSet(pedalboard_set_json) => {
                    match serde_json::from_str::<rs_pedalboard::pedalboard_set::PedalboardSet>(
                        &pedalboard_set_json,
                    ) {
                        Ok(pedalboard_set) => {
                            self.pedalboards
                                .active_pedalboardstage
                                .replace(pedalboard_set);
                        }
                        Err(e) => {
                            tracing::error!(
                                "Failed to parse pedalboard set JSON from processor: {}",
                                e
                            );
                        }
                    }
                ...
                Command::NextPedalboard => {
                    ...
                    let new_index =
                        (pedalboard_set.active_pedalboard + 1) % pedalboard_set.pedalboards.len();
                    drop(pedalboard_set);
                ...
                Command::AddPedalboard(pedalboard_json) => {
                    match serde_json::from_str::<Pedalboard>(&pedalboard_json) {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "Failed to parse pedalboard JSON from other thread: {}",
                                e
                            );
                        }
                    }
                ...
                Command::UtilitiesView => {
                    if self.selected_screen.get() == Screen::Utilities {
                        let switching_to =
                            self.prev_selected_screen.get().unwrap_or(Screen::Utilities);
                ...
                Command::SettingsView => {
                    if self.selected_screen.get() == Screen::Settings {
                        self.set_screen(
                            self.prev_selected_screen.get().unwrap_or(Screen::Settings),
                        );
                    } else {
                ...
                Command::SubscribeToResponses(_) => tracing::error!(
                    "Unexpected SubscribeToResponses command in other thread commands"
                ),
            }
        }
    }
