...
impl Display for AudioDeviceError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            AudioDeviceError::HostRequired => {
                write!(f, "Host is required to get audio devices on this platform")
            }
...
pub fn get_input_devices(host: Option<&Host>) -> Result<Vec<String>, AudioDeviceError> {
    if let Some(host) = host {
        let input_devices: Result<Vec<String>, AudioDeviceError> = host
            .input_devices()
            .map_err(AudioDeviceError::DevicesError)?
...
pub fn get_output_devices(host: Option<&Host>) -> Result<Vec<String>, AudioDeviceError> {
    if let Some(host) = host {
        let output_devices: Result<Vec<String>, AudioDeviceError> = host
            .output_devices()
            .map_err(AudioDeviceError::DevicesError)?
            .map(|d| d.name().map_err(AudioDeviceError::DeviceNameError))
            .collect();
