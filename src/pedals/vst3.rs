...
use crate::plugin::vst3::{Vst3Instance, VST3_PLUGIN_PATH};
...
pub const OVERRIDE_DEFAULT_FOLDERS_ENV_VAR: &str = "RSPEDALBOARD_VST3_FOLDER";

pub fn set_vst3_save_path(new_path: &str) {
    std::env::set_var(OVERRIDE_DEFAULT_FOLDERS_ENV_VAR, new_path);
}

/// A VST3 plugin hosted without embedding the plugin's native GUI.
///
/// Plugin parameters are exposed as normalized values that can be edited from
/// the generic parameter window, mirroring the VST2 pedal.
#[derive(Clone)]
pub struct Vst3 {
...
impl<'a> Deserialize<'a> for Vst3 {
        ...
        D: serde::Deserializer<'a>,
    {
        ...
        let helper = Vst3Data::deserialize(deserializer)?;
...
impl Vst3 {
    ...
    pub fn get_save_directory() -> Option<PathBuf> {
        if let Some(override_path) = std::env::var_os(OVERRIDE_DEFAULT_FOLDERS_ENV_VAR) {
            tracing::debug!(
                "Using overridden VST3 save directory from env var {}: {:?}",
    ...
    fn get_empty_directory_combo_box(id: impl std::hash::Hash) -> DirectoryComboBox {
        ...
        DirectoryComboBox::new_from_nodes(roots)
            .with_id(egui::Id::new("vst3_combobox").with(id_hasher.finish()))
            .with_wrap_mode(egui::TextWrapMode::Truncate)
            .show_extensions(false)
            // VST3 plugins are directories (bundles), so allow directories to be selected.
    ...
    pub fn set_plugin<P: AsRef<Path>>(&mut self, plugin_path: P) {
        ...
        let new_plugin_instance = Vst3Instance::load(canon_path);
    ...
    fn update_combobox_nodes(&mut self, ui: &mut egui::Ui) {
        ...
        let new_root_directories: Option<Vec<egui_directory_combobox::DirectoryNode>> =
            ui.ctx().memory_mut(|m| {
                let state = m.data.get_temp_mut_or("vst3_folders_state".into(), 1u32);
                if *state != self.folders_state {
                    self.folders_state = *state;
                    m.data.get_temp("vst3_folders".into()).as_ref().cloned()
                } else {
                    None
                }
            });
        ...
        if let Some(mut roots) = new_root_directories {
            if let Some(node) = DirectoryNode::try_from_path(VST3_PLUGIN_PATH) {
                roots.push(node);
            } else {
                tracing::warn!(
                    "Failed to get default VST3 save directory: {}",
                    VST3_PLUGIN_PATH
                );
            }
            let model_path = self
        ...
        if old.as_ref().map(|p| p.as_path()) != combobox_to_show.selected() {
            match combobox_to_show.selected() {
                Some(path) => match path.to_str() {
                    ...
                    None => {
                        tracing::warn!("Selected plugin path is not valid unicode");
    ...
    fn ui(
        ...
        ui.allocate_ui_with_layout(
            ...
            |ui| {
                ...
                ui.label(egui::RichText::new("VST3").size(23.0));
