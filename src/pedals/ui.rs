...
pub fn pedal_knob(
    ...
    let knob_rotate =
        KNOB_MIN_ANGLE + value_fract_between_min_max * (KNOB_MAX_ANGLE - KNOB_MIN_ANGLE);
    ...
    ui.scope_builder(
        ...
        |ui| {
            ...
            main_knob_im_ui.add(
                egui::Image::new(egui::include_image!("images/pedal_knob_blender_base.png"))
                    .rotate(knob_rotate, Vec2::splat(0.5))
                    .max_width(size_px)
                    .tint(tint),
            );

            let knob_im_shine_overlay = ui.add(
                egui::Image::new(egui::include_image!("images/pedal_knob_blender_shine.png"))
                    .max_width(size_px)
                    .sense(egui::Sense::click_and_drag()),
            );
            ...
            if knob_im_shine_overlay.dragged() {
                let current_y = ui
                    .input(|i| {
                        i.pointer
                            .interact_pos()
                            .expect("Failed to get cursor location")
                    })
                    .y;
                ...
                let (init_y, init_value) = if knob_im_shine_overlay.drag_started() {
                    ...
                    ui.ctx().memory_mut(|m| {
                        m.data
                            .insert_temp(Id::new("knob_drag_init_y"), (current_y, value));
                        (current_y, value)
                    })
                } else {
                    ui.ctx().memory(|m| {
                        m.data
                            .get_temp::<(f32, f32)>(Id::new("knob_drag_init_y"))
                            .unwrap_or((0.0, 0.0))
                            .clone()
                    })
                };
...
pub fn pedal_switch(ui: &mut egui::Ui, active: bool, at: egui::Vec2, height: f32) -> Option<bool> {
    ...
    egui::Image::new(switch_image).paint_at(ui, switch_rect);
...
pub fn sideways_arrow(ui: &mut egui::Ui, button_rect: egui::Rect, left: bool) {
    ...
    let points = if left {
        vec![
            right_arrow_rect.right_top(),
            right_arrow_rect.right_bottom(),
            right_arrow_rect.left_center(),
        ]
    } else {
        vec![
            right_arrow_rect.left_top(),
            right_arrow_rect.right_center(),
    ...
    let response = ui
        .vertical(|ui| {
            ...
            ui.columns_const(|[sine_ui, square_ui, sawtooth_ui, triangle_ui]| {
                if matches!(selected_oscillator, Oscillator::Sine(_)) {
                    ...
                    new_oscillator = Some(Oscillator::Sine(oscillator::Sine::new(
                        // Sample rate on oscillator parameters on client do not matter, it is set correctly on the processor
                        48000.0,
            ...
            if !oscillator_type_only {
                ...
                let frequency_range = {
                    let min_freq = parameter
                        .min
                        .as_ref()
                        .and_then(|p| p.as_float())
                        .unwrap_or(0.0);
                    let max_freq = parameter
                        .max
                        .as_ref()
                        .and_then(|p| p.as_float())
                        .unwrap_or(20.0);
                ...
                ui.add(
                    egui::Slider::new(&mut frequency_value, frequency_range)
                        .logarithmic(true)
                        .max_decimals(2),
                );
