        ...
        impl PedalTrait for $name {
            ...
            fn process_audio(&mut self, buffer: &mut [f32], _message_buffer: &mut Vec<String>) {
                if self.variable_delay_phaser.is_none() {
                    tracing::error!(
                        "{}: VariableDelayPhaser is not initialized. Call set_config() first.",
                        stringify!($name)
                    );
                    return;
                }
                self.variable_delay_phaser
                    .as_mut()
                    .unwrap()
                    ...
                    "Min Depth" => {
                        if let PedalParameterValue::Float(depth) = value {
                            ...
                            self.parameters.get_mut(name).unwrap().value =
                                PedalParameterValue::Float(bounded_depth);
            ...
            fn ui(
                ...
                if let Some(value) = pedal_knob(
                    ...
                    "Min Depth",
                    min_depth_param,
                    eframe::egui::Vec2::new(0.086, 0.036),
                    0.3,
                    self.id,
                ) {
                ...
                if let Some(value) = pedal_knob(
                    ...
                    "Max Depth",
                    max_depth_param,
                    eframe::egui::Vec2::new(0.61, 0.036),
                    0.3,
                    self.id,
                ) {
                ...
                if $incl_feedback {
                    ...
                    if let Some(value) = pedal_knob(
                        ...
                        "Dry/Wet",
                        dry_wet_param,
                        eframe::egui::Vec2::new(0.605, 0.3),
                        0.3,
                        self.id,
                    ) {
                ...
                let active_param = self
                    ...
                    .unwrap();
                if let Some(value) =
                    pedal_switch(ui, active_param, egui::Vec2::new(0.33, 0.72), 0.16)
                {
            ...
            fn set_config(&mut self, _buffer_size: usize, sample_rate: u32) {
                ...
                self.variable_delay_phaser = Some(VariableDelayPhaser::new(
                    min_depth,
                    max_depth,
                    dry_wet,
                    variable_delay_phaser_oscillator,
                    feedback,
                    sample_rate as f32,
                ));
            }
